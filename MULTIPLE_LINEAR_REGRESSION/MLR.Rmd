...
library(sf)
library(dplyr)
census_tracts <- st_read("california.shp", quiet = TRUE)

```

```{r }

latlong_dat <- df[,c(1,2)]
str(lat_long_dat)
```
...
system.time({
  for(i in 1:11){
  census_block <- tigris::call_geolocator_latlon(lat = latlong_dat$latitude[i],
                                                 lon = latlong_dat$longitude[i])
  }
  
  (census_tract <- substr(census_block[i], start = 1, stop = 11))
