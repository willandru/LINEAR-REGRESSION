...
x.train<-sample_frac(tbl=cdf,replace=FALSE,size=0.80)                                                       x.test<-anti_join(cdf,x.train)
 
                                                      
 model2<-lm(x.train$median_house_value~x.train$log_total_rooms + x.train$latitud  + x.train$median_income + x.train$ocean_proximity_1H_OCEAN  + x.train$ocean_proximity_inland + x.train$beedrooms_per_room  +  x.train$log_population_per_household  ,x.train)
```
...
```{r }
 ...
 test_predictions=predict(model2,x.test)         
