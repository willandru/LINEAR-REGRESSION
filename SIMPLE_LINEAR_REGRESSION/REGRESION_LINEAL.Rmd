...
```{r }
plot(yes$bmi, yes$charges, main = "Modelo CON intercepto para SI FUMADORES", xlab = 'BMI', ylab = 'CHARGES')+
abline( -13186.58,  1473.11)
```
```{r }
plot(yes$bmi, yes$charges,main = "Modelo SIN intercepto para SI FUMADORES", xlab = 'BMI', ylab = 'CHARGES')+
abline( 0, 1061.08)
```
...
## ANALISIS DE VARIANZAS PARA LOS MODELOS EN GENERAL
...
Observando las relaciones de la variables de interes con las otras variables en los diagramas y analizando su comportamiento con respecto a las variables categoricas, pudímos darnos cuenta de **relaciones lineales importantes** que nos permiten analizar por separado el problema de regresión lineal de la variable **'charges'**.
...
UNA SOLUCIÓN MÁS GENERAL sería sacar los outlayers del modelo de AGE vs CHARGE y obtener un mejor ajuste de los datos que estan en gráficamente se ven en la corriente principal suponiendo los otros datos como atipicos. 